...
import { Loader2, Save, FileSearch, Printer, CheckCircle2, User, Users, MapPin, Settings, Type, Hash, Calendar, Clock, Wind, Gauge, Thermometer, Droplets, Battery, Zap, Mic, MicOff, Camera, ClipboardList, FileText, Image as ImageIcon, Wand2, X } from 'lucide-react';
...
export const generatePDF = async (reportRaw: any, inspectorName: string, reportId: string | null) => {
  ...
  try {
    ...
    const displayID = (individualID && individualID !== finalID && finalID !== 'BORRADOR')
      ...
      : finalID;
    doc.text("INFORME DE REVISIÓN", leftMargin, currentY);
    doc.setFontSize(12);
    ...
    const colWidth = 28;
    const checklistSections = Object.entries(CHECKLIST_SECTIONS);
    const electricalSectionIndex = checklistSections.findIndex(([section]) => section === 'INSPECCION EQUIPO ELECTRICO');
    const firstChecklistSections = electricalSectionIndex >= 0
      ? checklistSections.slice(0, electricalSectionIndex)
      : checklistSections;
    const secondChecklistSections = electricalSectionIndex >= 0
      ? checklistSections.slice(electricalSectionIndex)
      : [];

    const buildChecklistBody = (sections: [string, any][]) => sections.flatMap(([section, items]) => {
    ...
    const renderChecklistTable = (sections: [string, any][], startY: number) => autoTable(doc, {
      startY,
      head: [['INSPECCIÓN / ESTADO', 'OK', 'DEFECT', 'CAMBIO']],
      body: buildChecklistBody(sections),
      theme: 'grid',
    ...
    renderChecklistTable(firstChecklistSections, currentY);

    if (secondChecklistSections.length > 0) {
      doc.addPage();
      currentY = topMargin;
      renderChecklistTable(secondChecklistSections, currentY);
    }
...
export default function InformeRevisionForm({ 
  ...
  const linkedOrderId = getLinkedOrderId(initialData);
  ...
  const handleClientSelect = (client: any) => {
    setFormData((prev: any) => ({
      ...
      motor: client.motor || prev.motor,
      modelo: client.modelo || prev.modelo,
      n_motor: client.n_motor || prev.n_motor,
      n_grupo: client.n_grupo || prev.n_grupo,
      potencia: client.potencia || prev.potencia,
    }));
  };
  ...
  const handleCaptureLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        let msg = 'Por favor active los permisos de ubicación.';
  ...
  // ########## PDF AND SIGNATURE ACTIONS ##########
  const handlePdfAction = (forceDownload = false, docIdOverride?: string) => {
    if (pdfLoading) return;
    if (!formData.clienteId) {
      toast({ variant: 'destructive', title: 'Faltan datos', description: 'Selecciona un cliente para generar el PDF.' });
      return;
    }
    setPdfLoading(true);
    setTimeout(() => {
      try {
        ...
        const rawId = getSafeReportId(formData.numero_informe) || getSafeReportId(docIdOverride) || (isSaved ? getSafeReportId(savedDocId) : '') || 'BORRADOR';
    ...
    try {
      ...
      if (canUseCloud && firestore && user?.email) {
        try {
          ...
          pushCounterToCloud('informe-revision', currentEmail || '', firestore, sequence).catch(() => {});
        ...
        await saveDataToLocal(false, docId);
        // Offline: intentar empuje si hay firestore disponible (se reintentará en la próxima sincronización)
      ...
      <main className="space-y-6 px-4 pt-4">
        ...
        <section className="bg-white p-5 md:p-8 rounded-[2rem] shadow-sm space-y-4 border border-slate-100">
          ...
          <div className="flex justify-between items-center border-b border-slate-100 pb-1.5">
            <h3 className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Hallazgos y Comentarios</h3>
          ...
          />
          <textarea className="w-full min-h-[220px] bg-slate-50 border border-slate-200 rounded-xl p-4 resize-y outline-none focus:border-primary focus:bg-white transition-all shadow-inner text-sm font-medium text-black leading-relaxed whitespace-pre-wrap" placeholder="Escriba aquí cualquier anomalía o trabajo adicional recomendado..." value={formData.observaciones} onChange={(e: any) => handleInputChange('observaciones', e.target.value)} spellCheck="true" lang="es" autoCorrect="on" autoCapitalize="sentences" />
          <div className="grid md:grid-cols-2 gap-6 items-start pt-4">
            <div className="text-left">
              <SignaturePad title="Firma del Inspector Técnico" signature={inspectorSignature} onSignatureEnd={setInspectorSignature} showSavedSignature={true} />
