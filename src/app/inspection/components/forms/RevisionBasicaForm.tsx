...
import { Loader2, Save, FileSearch, Printer, CheckCircle2, User, Users, MapPin, Settings, Type, Hash, Calendar, Clock, Wind, Gauge, Thermometer, Droplets, Battery, Zap, Wrench, Camera, ClipboardList, FileText, Image as ImageIcon, X, Mic, MicOff, Wand2 } from 'lucide-react';
...
export const generatePDF = async (reportRaw: any, inspectorName: string, reportId: string | null) => {
  ...
  const displayID = (individualID && individualID !== finalID && finalID !== 'BORRADOR') 
    ...
    : finalID;
  doc.text("REVISIÓN BÁSICA", leftMargin, currentY);
  doc.setFontSize(12);
...
export default function RevisionBasicaForm({ 
  ...
  const handleCaptureLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        let msg = 'Asegúrate de tener los permisos de localización activados.';
        if (error.code === error.TIMEOUT) msg = 'Tiempo de espera agotado buscando GPS.';
        toast({ variant: 'destructive', title: 'Ubicación denegada', description: msg });
        setLocationStatus('error');
      },
  ...
  const handleEnhanceReport = async () => {
    ...
    try {
      ...
      setFormData((prev: any) => {
        ...
        const recambiosUpdates = { ...prev.recambios };
        const checklistUpdates = res.checklist_updates || {};
        const recambiosMapping: { [key: string]: string } = {
          'Filtro de aceite': 'fa',
          'Filtro de combustible': 'fc',
          'Filtro de aire': 'far',
          'Filtro de agua': 'fag',
    ...
    try {
      ...
      if (isEditingExisting && savedDocId) {
        ...
        if (canUseCloud && firestore) {
          ...
          updatePayload.clientSignatureUrl = includeClientSignature ? clientSignatureUrl : null;
      ...
      if (canUseCloud && firestore && user?.email) {
        try {
          ...
          pushCounterToCloud('revision-basica', currentEmail || '', firestore, sequence).catch(() => {});
        } catch (e) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 w-full bg-white min-h-screen pb-20">
      ...
      <main className="space-y-8 pb-40 px-4 pt-4">
        ...
        <section className="bg-white p-5 md:p-8 rounded-[2rem] shadow-sm space-y-4 border border-slate-100">
          ...
          <div className="flex justify-between items-center border-b border-slate-100 pb-1.5">
            <h3 className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Hallazgos y Observaciones</h3>
